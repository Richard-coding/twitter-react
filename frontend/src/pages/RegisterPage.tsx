...
const inputBase =
  "rounded-xl px-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none transition-all";
const inputStyle = {
...
export default function RegisterPage() {
  ...
  const [isInputUsernameOnFocus, setIsInputUsernameOnFocus] = useState(false);
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Crie sua conta e faça parte da nossa comunidade. Posts, eventos,
            filmes e muito mais.
          </p>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 py-10">
          ...
          <p className="text-sm text-slate-500 mb-8">
            Preencha os campos abaixo para entrar na casa.
          </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                style={{ ...inputStyle }}
                onFocus={() => {
                  setIsInputUsernameOnFocus(true);
                }}
                onBlur={() => setIsInputUsernameOnFocus(false)}
              />

              {(isInputUsernameOnFocus || /^[a-zA-Z0-9_]+$/.test(username)) && (
                <ul>
                  <li
                    className={`${username.length >= 4 ? "text-green-500" : " text-red-500"} `}
                  >
                    Minimo de caracteres 4-15
                  </li>
                  <li
                    className={`${/^[a-zA-Z0-9_]+$/.test(username) ? "text-green-500" : " text-red-500"} `}
                  >
                    Permitido apenas letras a-z *
                  </li>
                  <li
                    className={`${/\d/.test(username) ? "text-green-500" : " text-yellow-500"} `}
                  >
                    Números 0-9{" "}
                  </li>
                  <li
                    className={`${/_/.test(username) ? "text-green-500" : " text-yellow-500"} `}
                  >
                    {" "}
                    Underline (_)
                  </li>
                  <li
                    className={`${/[^a-zA-Z0-9_]/.test(username) ? "text-red-500" : " text-white"} `}
                  >
